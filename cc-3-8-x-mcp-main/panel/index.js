...
<div class="wrap">
  <section class="mcp">
    <header>MCP 服务 <span id="mcpDot" class="dot gray"></span></header>
    <div class="row"><label>状态</label><span id="mcpRunning">-</span></div>
    <div class="row"><label>端点</label><span id="mcpUrl">-</span></div>
    <div class="row"><label>工具</label><span id="mcpTools">-</span></div>
  ...
  <section class="status">
    <header>编辑器状态 <span id="probeDot" class="dot gray" title="预览连通性"></span></header>
    ...
    <div class="row"><label>监听器</label><span id="watchers">-</span></div>
    ...
    async refreshStatus() {
        try {
            ...
            if (mcp.running) {
                ...
                this.$.mcpRunning.textContent = '运行中';
                this.$.mcpUrl.textContent = mcp.url || '-';
                this.$.mcpTools.textContent = (mcp.toolCount || 0) + ' 个工具 / ' + (mcp.resourceCount || 0) + ' 个资源';
                this.$.mcpReqCount.textContent = String((mcp.stats && mcp.stats.requestCount) || 0);
            } else {
                this.$.mcpDot.className = 'dot red';
                this.$.mcpRunning.textContent = '已停止';
