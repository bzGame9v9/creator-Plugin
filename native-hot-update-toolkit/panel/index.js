...
exports.template = [
  ...
  '      <div class="publish-step"><strong>第一步：上传本次新增 Bundle Hash</strong><span>打开 bundle_current，将除 manifest 外的 Bundle 目录内容覆盖到 CDN 的 bundle/。</span><ui-button id="openBundleCurrent">打开 bundle_current</ui-button></div>',
  '      <div class="publish-step"><strong>第二步：上传版本 Manifest</strong><span>把当前 manifest 文件上传到 CDN 的 version/。</span><ui-button id="copyVersionManifest">复制 Manifest 文件</ui-button></div>',
...
exports.$ = {
  ...
  openBundleCurrent: '#openBundleCurrent', copyVersionManifest: '#copyVersionManifest', copyBackendJson: '#copyBackendJson',
...
function renderState(panel) {
  ...
  panel.$.publishSummary.textContent = publish
    ? ['当前发布：' + publish.releaseId, '本次目录：' + publish.bundleCurrentRoot, '版本 Manifest：' + publish.manifestFile, '后台 JSON：' + publish.backendFile].join('\n')
    : '还没有可发布的资源结果。';
  panel.$.openBundleCurrent.disabled = !publish || !!state.busy;
  panel.$.copyVersionManifest.disabled = !publish || !!state.busy;
  ...
  panel.$.completionPaths.textContent = completion ? [
    ...
    completion.bundleCurrentRoot ? '本次发布目录：' + completion.bundleCurrentRoot : '',
...
exports.ready = async function ready() {
  ...
  this.$.openBundleCurrent.addEventListener('confirm', function () {
    void openPath(panel, currentState && currentState.publish && currentState.publish.bundleCurrentRoot);
  });
