...
function readCompletion(root, mode, environment) {
    ...
    return {
        ...
        componentReleaseFile: componentRelease && componentRelease.releaseFile || '',
        changedComponents: componentRelease && componentRelease.changedComponents || [],
        componentUploadPaths: componentRelease && componentRelease.uploadPaths || [],
        bundleCurrentRoot: componentRelease && completed.bundleCurrent && completed.bundleCurrent.root || '',
    };
}
...
function resolvePublishArtifacts(root, raw) {
    ...
    const outputRoot = resolveConfiguredPath(root, raw.environments[environment].outputRoot);
    const releaseId = path.basename(release.releaseFile, '.manifest');
    const bundleCurrentRoot = path.join(outputRoot, 'bundle_current');
    const manifestFile = path.join(bundleCurrentRoot, path.basename(release.releaseFile));
    const backendFile = path.join(outputRoot, 'backend-config', `${releaseId}.json`);
    return {
        environment,
        releaseId,
        bundleCurrentRoot,
